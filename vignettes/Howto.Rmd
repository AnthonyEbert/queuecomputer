...
The resourcing schedule is specified with either a non-zero natural number, a `server.step` or a `server.list` object. Use a non-zero natural number when the number of servers does not change over time. The `server.step` object is used when we don't care which particular servers are available, just how many are available at certain times. The computation speed for `queue_step()` is much faster when using a `server.step` rather than a `server.list` input for the `servers` argument. 

We create a `server.step` object with the `as.server.step` function. 
...
resource_schedule <- as.server.step(c(10,50,100), c(0, 1, 3, 1))
